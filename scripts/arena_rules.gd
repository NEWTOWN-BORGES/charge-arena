...
func can_fire(team: int, dt: float = 0.0) -> bool:
	# Whether a shot asked for on the tick about to run would leave the barrel. A click is
	# kept until this says yes, so one pressed during the reload is not thrown away.

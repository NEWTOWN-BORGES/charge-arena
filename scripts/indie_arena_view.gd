...
func measure_view_bounds() -> Rect2:
	# Stadium extent on the camera plane, side beacons included and soft shadows ignored.
...
func frame_rect(rect: Rect2, screen: Vector2) -> void:
	# Fit the whole stadium inside a screen rectangle, e.g. the band between portrait HUD cards.
	view_aspect()
	var units_per_pixel = maxf(view_bounds.size.x / rect.size.x, view_bounds.size.y / rect.size.y) * 1.015

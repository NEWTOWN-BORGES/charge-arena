...
func run() -> void:
	...
	check(ultimates == ["beacon", "charges", "glass", "sentries", "bloom", "plunder", "thunder", "meteors", "singularity", "sun_ray"], "Every boss brings one of its own, in order: beacon, charges, glass, sentries, bloom, plunder, thunder, meteors, singularity, sun ray")
	check(String(Skins.CATALOG[levels[0].boss].ultimate) == "surge", "And the training pilot brings the overload every player starts with")

...
func draw_icon() -> void:
	...
	portraits.sentinel_portrait(painter, Vector2.ZERO, preload("res://scripts/skins.gd").colors(5, Color.WHITE), false)
	...
	painter.draw_string(font, Vector2(232, 417), "LAB", HORIZONTAL_ALIGNMENT_LEFT, -1, 25, Color("101d29"))
func run() -> void:
	...
	for name in ["icon", "foreground"]:
		...
		var error = root.get_texture().get_image().save_png("res://art/sentinel_lab_%s.png" % name)
		if error != OK:
			quit(error)
			return
	portraits.free()
